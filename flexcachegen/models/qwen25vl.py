...
from transformers.models.qwen2_5_vl.modeling_qwen2_5_vl import Qwen2_5_VisionTransformerPretrainedModel, Qwen2_5_VLRotaryEmbedding, Qwen2RMSNorm

from flash_attn import flash_attn_func, flash_attn_with_kvcache

from flexcachegen.config import Config
from flexcachegen.kvcache import KVCacheManager
from flexcachegen.models.qwen25vl_util import get_video_features, get_placeholder_mask, get_rope_index
from flexcachegen.utils import print_duration



class Qwen25VLTextInputEmbed(nn.Module):
...
class Qwen25VLTextAttention(nn.Module):
    def __init__(self, config: Config, layer_idx: int):
        ...
        self.input_layernorm = Qwen2RMSNorm(self.hidden_size, eps=self.rms_norm_eps)
        self.q_proj = nn.Linear(self.hidden_size, self.hidden_size, bias=True)
        self.k_proj = nn.Linear(self.hidden_size, self.kv_hidden_size, bias=True)
        self.v_proj = nn.Linear(self.hidden_size, self.kv_hidden_size, bias=True)
    ...
    def apply_rotary_pos_emb(self, q, k, cos, sin, mrope_section, unsqueeze_dim=1):
...
class Qwen25VLTextMLP(nn.Module):
    def __init__(self, config: Config):
        super().__init__()
        self.config = config
...
class Qwen25VLOutputHead(nn.Module):
    def __init__(self, config: Config):
        super().__init__()
        self.config = config
...
class Qwen25VLDecoderLayer(nn.Module):
    def __init__(self, config: Config, layer_idx: int):
        super().__init__()
        self.self_attn = Qwen25VLTextAttention(config, layer_idx)
        self.mlp = Qwen25VLTextMLP(config)

    def forward(self):
        raise NotImplementedError("Use specific layers for generating.")


class Qwen25VLTextModel(nn.Module):
    def __init__(self, config: Config):
        super().__init__()
        self.config = config
        ...
        # components
        self.input_embed = Qwen25VLTextInputEmbed(config)
        self.rotary_emb = Qwen2_5_VLRotaryEmbedding(config.hf_config.text_config)
        self.layers = nn.ModuleList(
            [Qwen25VLDecoderLayer(config, layer_idx) for layer_idx in range(self.num_hidden_layers)]
        )
        self.output_head = Qwen25VLOutputHead(config)
...
class Qwen25VLModel(nn.Module):
    ...
    def __init__(self, config: Config, kv_cache_manager: KVCacheManager):
        ...
        # components
        self.visual_model = Qwen2_5_VisionTransformerPretrainedModel._from_config(config.hf_config.vision_config) # use origin implementation
        self.language_model = Qwen25VLTextModel(config)
        ...
        if os.path.exists(processed_visual_model_path) and os.path.exists(processed_language_model_path):
            ...
            for f in weight_files:
                ...
                for k, v in shard.items():
                    print(f"Processing weight: {k} from file: {f}")
    ...
    def encoding(self, inputs):
        ...
        second_per_grid_ts = inputs["second_per_grid_ts"].to(self.config.device)
        ...
        video_embeds = get_video_features(self.visual_model, pixel_values_videos, video_grid_thw)
